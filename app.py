...
# V10 on-demand large-file prefix cache.  The first part of a large file is fetched in
# the background immediately after the link is created.  HTTP requests can
# read the growing cache, so the player does not have to wait for a whole
# Telegram transfer before receiving the first bytes.
STREAM_CACHE_DIR = os.getenv("STREAM_CACHE_DIR", os.path.join(BASE_DIR, ".stream_cache"))
STREAM_CACHE_WARM_BYTES = int(os.getenv("STREAM_CACHE_WARM_BYTES", str(16 * 1024 * 1024)))
STREAM_CACHE_MAX_FILES = int(os.getenv("STREAM_CACHE_MAX_FILES", "12"))
CACHE_TASKS = {}
CACHE_CONDITIONS = {}
...
def increment_total_links():
    """Best-effort lifetime counter; a missing bot_stats table must not create an app error."""
    try:
        rows = sb_request(
            "GET", "bot_stats",
...
async def make_link(message):
    ...
    # Bot API's copyMessage response contains a reusable file_id.  Keeping it
    # in the link row lets <=20 MB files stream without touching the Pyrogram
    # media session at all.
    copied_file_id = None
    for media_key in ("video", "document", "audio"):
        if isinstance(copied.get(media_key), dict):
            copied_file_id = copied[media_key].get("file_id")
            if copied_file_id:
                break
    if not copied_file_id and isinstance(copied.get("photo"), list) and copied["photo"]:
        copied_file_id = copied["photo"][-1].get("file_id")
    ...
    await asyncio.to_thread(insert_link, link_item)
    # IMPORTANT: do not start an MTProto warm-up when the link is created.
    # That background transfer was competing with the first playback request
    # and could block even small files through Pyrogram reconnects.
    try:
...
def media_file_id(message):
    # V10 small-file fast path: link rows keep the Bot API file_id, so no
    # Pyrogram get_messages() call is needed for <=20 MB files.
    if isinstance(message, dict):
        direct = message.get("bot_file_id")
        if direct:
            return direct
...
def _cache_key(item_or_chat_id, message_id=None):
    if isinstance(item_or_chat_id, dict):
        chat_id = item_or_chat_id.get("storage_chat_id")
        message_id = item_or_chat_id.get("storage_message_id")
    else:
        chat_id = item_or_chat_id
    return hashlib.sha1(f"{chat_id}:{message_id}".encode()).hexdigest()


def _cache_path(key):
    return os.path.join(STREAM_CACHE_DIR, key + ".bin")


def _cache_condition(key):
    with CACHE_STATE_LOCK:
        cond = CACHE_CONDITIONS.get(key)
        if cond is None:
            cond = threading.Condition()
            CACHE_CONDITIONS[key] = cond
        return cond


def _cache_size(key):
    try:
        return os.path.getsize(_cache_path(key))
    except OSError:
        return 0
    ...
    try:
        ...
        for name in os.listdir(STREAM_CACHE_DIR):
            ...
            try:
                ...
                pass
        if len(files) <= STREAM_CACHE_MAX_FILES:
            return
        files.sort()
        for _, path in files[:-STREAM_CACHE_MAX_FILES]:
            try:
                os.remove(path)
            except OSError:
                pass
...
async def warm_large_file_cache(item, warm_bytes=STREAM_CACHE_WARM_BYTES):
    """Warm only the beginning of a large file; never wait for the whole file."""
    if int(item.get("size") or 0) <= BOT_API_MAX_DOWNLOAD:
        return

    key = _cache_key(item)
    existing = _cache_size(key)
    if existing >= min(warm_bytes, int(item.get("size") or warm_bytes)):
        return

    with CACHE_STATE_LOCK:
        current = CACHE_TASKS.get(key)
        if current and not current.done():
            return
        CACHE_TASKS[key] = asyncio.current_task()

    try:
        msg = await get_storage_message(item)
        target = min(warm_bytes, int(item.get("size") or warm_bytes))
        start_chunk = existing // CHUNK_SIZE
        skip = existing % CHUNK_SIZE
        written = existing
        path = _cache_path(key)
        log.info("CACHE WARM START key=%s existing=%s target=%s", key[:10], existing, target)

        async with MT_PROTO_STREAM_LOCK:
            for attempt in range(MT_PROTO_MAX_RETRIES + 1):
                try:
                    with open(path, "ab") as fh:
                        async for chunk in bot.stream_media(msg, offset=start_chunk):
                            if skip:
                                if skip >= len(chunk):
                                    skip -= len(chunk)
                                    continue
                            ...
                            if chunk:
                                ...
                                fh.flush()
                                written += len(chunk)
                                with _cache_condition(key):
                                    _cache_condition(key).notify_all()
                            if written >= target:
                                break
                    break
                except Exception as exc:
                    wait_seconds = int(getattr(exc, "value", 0) or 0)
                    if "FLOOD_WAIT" in str(exc).upper() and wait_seconds and attempt < MT_PROTO_MAX_RETRIES:
                        wait_seconds = min(wait_seconds, 15)
                        log.warning("CACHE WARM FloodWait %ss; retrying", wait_seconds)
                        await asyncio.sleep(wait_seconds)
                        continue
                    raise

        log.info("CACHE WARM READY key=%s cached=%s", key[:10], written)
        _cache_cleanup()
    except asyncio.CancelledError:
        raise
    except Exception:
        log.exception("CACHE WARM FAILED key=%s", key[:10])
    finally:
        with _cache_condition(key):
            _cache_condition(key).notify_all()
        with CACHE_STATE_LOCK:
            task = CACHE_TASKS.get(key)
            if task is asyncio.current_task():
                CACHE_TASKS.pop(key, None)


def start_cache_warm(item):
    # Disabled in V10. Warm-up at link creation caused MTProto contention.
    return


def serve_cached_prefix(wfile, key, start, length, timeout=8.0):
    """Serve bytes already/warmingly available in the prefix cache."""
    path = _cache_path(key)
    ...
    while remaining > 0:
        available = _cache_size(key)
        if available > position:
            take = min(available - position, remaining, CHUNK_SIZE)
...
async def stream_producer(message, out_queue, stop_event, start_chunk, first_skip, wanted_bytes):
    """V8 MTProto producer with serialized media transfers and FloodWait retry."""
    sent = 0
    ...
    log.info(
        "STREAM START offset_chunk=%s first_skip=%s wanted=%s",
        start_chunk, first_skip, wanted_bytes,
    )

    try:
        # Do not let several browser/MX/VLC Range requests create Telegram
        # media-DC authorizations at the same time.
        async with MT_PROTO_STREAM_LOCK:
            for attempt in range(MT_PROTO_MAX_RETRIES + 1):
                if stop_event.is_set():
                    break
                try:
                    ...
                    log.info(
                        "STREAM PRODUCER FINISHED sent=%s/%s attempt=%s",
                        sent, wanted_bytes, attempt + 1,
                    )
                    break
                    ...
                    # Pyrogram/Telegram FloodWait exposes the wait in seconds.
                    wait_seconds = int(getattr(exc, "value", 0) or 0)
                    if "FLOOD_WAIT" in str(exc).upper() and wait_seconds and attempt < MT_PROTO_MAX_RETRIES:
                        wait_seconds = min(wait_seconds, 60)
                        log.warning(
                            "MTProto FloodWait: waiting %ss before retry %s/%s",
                            wait_seconds, attempt + 1, MT_PROTO_MAX_RETRIES,
                        )
                        await asyncio.sleep(wait_seconds)
                        continue
                    raise
...
class Handler(BaseHTTPRequestHandler):
    ...
    def serve_media(self, item, download=False):
        try:
            ...
            # Telegram Bot API can download files up to 20 MB.  Do NOT call
            # Pyrogram get_messages() for this path; that was the reason even
            # 8 MB/20 MB files became slow after the V9 warm-cache change.
            msg = None
            ...
            if size <= BOT_API_MAX_DOWNLOAD:
                msg = {"bot_file_id": item.get("bot_file_id")}
            ...
            # Large files: consume the growing prefix cache first.  If the
            # requested range is not in that cache (for example a far seek),
            # fall back to the existing direct MTProto Range stream.
            key = _cache_key(item)
            start_cache = start
            if start_cache < STREAM_CACHE_WARM_BYTES:
                log.info("MEDIA PATH=CACHE_PREFIX size=%s start=%s length=%s", size, start, length)
                cached_sent = serve_cached_prefix(self.wfile, key, start, length, timeout=8.0)
                if cached_sent >= length:
                    return
                start_cache += cached_sent
                length -= cached_sent
                start = start_cache
                start_chunk = start // CHUNK_SIZE
                first_skip = start % CHUNK_SIZE

            # Only large files need Pyrogram's storage message.  Fetch it
            # after the prefix-cache attempt so a cached seek never touches
            # the Telegram MTProto session.
            msg = get_storage_message(item)
            producer = stream_producer(msg, q, stop_event, start_chunk, first_skip, length)
            log.info("MEDIA PATH=MTProto DIRECT size=%s start=%s length=%s", size, start, length)
            future = asyncio.run_coroutine_threadsafe(producer, MAIN_LOOP)
