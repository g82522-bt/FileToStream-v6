...
# V9 large-file warm cache.  The first part of a large file is fetched in
...
def increment_total_links():
    """Increment the stats row without relying on a missing RPC function."""
    rows = sb_request(
        "GET", "bot_stats",
...
async def make_link(message):
    ...
    if size > BOT_API_MAX_DOWNLOAD:
        # Start warming immediately so the first player/download request can
        # consume the growing cache instead of waiting for a new MTProto
        # authorization/transfer to finish. Prime logic is untouched.
        asyncio.create_task(warm_large_file_cache(link_item))
    try:
...
def start_cache_warm(item):
    if int(item.get("size") or 0) <= BOT_API_MAX_DOWNLOAD or MAIN_LOOP is None:
        return
    asyncio.run_coroutine_threadsafe(warm_large_file_cache(dict(item)), MAIN_LOOP)
