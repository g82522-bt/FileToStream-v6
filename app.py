...
log = logging.getLogger("true-stream-v8")
...
# Large Telegram files use Pyrogram MTProto. Telegram can rate-limit
# repeated media-DC authorization when several Range requests start at once.
# V8 serializes MTProto media transfers and retries FloodWait safely.
MT_PROTO_STREAM_LOCK = asyncio.Lock()
MT_PROTO_MAX_RETRIES = 2

# Large-file cache: one Telegram MTProto download per storage message.
# HTTP Range requests are served from this local cache instead of calling
# bot.stream_media() again for every browser request. This is the key fix for
# repeated auth.ExportAuthorization / FLOOD_WAIT on media DC4.
STREAM_CACHE_DIR = os.getenv("STREAM_CACHE_DIR", os.path.join(BASE_DIR, ".stream_cache"))
STREAM_CACHE_MAX_BYTES = int(os.getenv("STREAM_CACHE_MAX_BYTES", str(2 * 1024 * 1024 * 1024)))
STREAM_CACHE_TTL = int(os.getenv("STREAM_CACHE_TTL", str(6 * 60 * 60)))
os.makedirs(STREAM_CACHE_DIR, exist_ok=True)
CACHE_ENTRIES = {}
CACHE_MAP_LOCK = threading.Lock()

# Keep the storage Message object in memory after the first lookup.
# This avoids another Pyrogram get_messages() call for every HTTP Range request
# and does not require any Supabase schema change.
STORAGE_MESSAGE_CACHE_TTL = 30 * 60
STORAGE_MESSAGE_CACHE = {}
STORAGE_MESSAGE_CACHE_LOCK = threading.Lock()
...
def increment_total_links():
    """Optional counter. If bot_stats is unavailable, silently use links as the source of truth."""
    try:
        rows = sb_request(
            "GET", "bot_stats",
            params={"id": "eq.1", "select": "total_links_generated", "limit": "1"},
        )
        current = int(rows[0].get("total_links_generated") or 0) if rows else 0
        updated = current + 1
        if rows:
            sb_request(
                "PATCH", "bot_stats",
                params={"id": "eq.1"},
                body={"total_links_generated": updated},
                prefer="return=minimal",
            )
        return updated
    except urllib.error.HTTPError as exc:
        if exc.code == 404:
            # bot_stats is optional; links remains the authoritative fallback.
            return None
        raise
...
def get_total_links_generated():
    try:
        ...
        return int(rows[0]["total_links_generated"]) if rows else sb_count("links")
    except urllib.error.HTTPError as exc:
        if exc.code == 404:
            return sb_count("links")
        raise
...
def tg_set_commands():
    commands = [
        ...
        {"command": "help", "description": "How True Stream works"},
...
async def make_link(message):
    ...
    # Warm only the lightweight Pyrogram Message metadata in the background.
    # Do NOT warm/download media here: that was the source of the V9 startup
    # slowdown. This keeps the first large-file playback path demand-driven.
    asyncio.create_task(warm_storage_message({
        "storage_chat_id": str(STORAGE_CHAT_ID),
        "storage_message_id": storage_message_id,
    }))
...
async def handle_message(message):
    ...
    if command == "/help":
        help_text = (
            "🆘 <b>True Stream Help</b>\n\n"
            "<b>Bot kaise kaam karta hai?</b>\n"
            "1️⃣ Video / Photo / Document / Audio isi bot ko bhejiye.\n"
            "2️⃣ Bot file ko secure storage me rakhega.\n"
            "3️⃣ Aapko ek streaming webpage link milega.\n"
            "4️⃣ Link ko browser me khol sakte hain ya MX Player / VLC se play kar sakte hain.\n\n"
            "<b>Free Limit</b>\n"
            "🆓 Free users ko rolling 24 hours me 3 links milte hain.\n"
            "💎 Prime users unlimited links bana sakte hain.\n"
            "👑 Admin ke liye unlimited access hai.\n\n"
            "<b>Playback</b>\n"
            "⏳ Free streaming me video start hone me lagbhag 15–39 seconds lag sakte hain.\n"
            "Badi files ko start hone me thoda zyada time lag sakta hai.\n\n"
            "<b>Problem ho to</b>\n"
            "• Link ko ek baar refresh/reopen karein.\n"
            "• Browser me play na ho to MX Player ya VLC try karein.\n"
            "• Bahut badi file ho to kuch seconds wait karein.\n"
            "• Link expire ho gaya ho to file dobara bhejkar naya link banayein.\n"
            "• Phir bhi problem rahe to <b>/help</b> message ke saath problem aur link admin ko bhejein."
        )
        await tg_send_message_async(user_id, help_text)
        return
...
def get_storage_message(item):
    key = (str(item.get("storage_chat_id")), int(item.get("storage_message_id")))
    now = time.time()
    with STORAGE_MESSAGE_CACHE_LOCK:
        cached = STORAGE_MESSAGE_CACHE.get(key)
        if cached and now - cached[0] < STORAGE_MESSAGE_CACHE_TTL:
            return cached[1]
        if cached:
            STORAGE_MESSAGE_CACHE.pop(key, None)
    ...
    with STORAGE_MESSAGE_CACHE_LOCK:
        STORAGE_MESSAGE_CACHE[key] = (time.time(), message)
    return message

async def warm_storage_message(item):
    try:
        await asyncio.to_thread(get_storage_message, item)
    except Exception:
        # This is only an optimization. Link creation and playback must not
        # fail if the background metadata lookup is unavailable.
        log.warning("Background storage message cache warm failed", exc_info=True)
...
def delete_storage_message(item):
    ...
    message_id = int(item["storage_message_id"])
    key = (str(item.get("storage_chat_id")), message_id)
    with STORAGE_MESSAGE_CACHE_LOCK:
        STORAGE_MESSAGE_CACHE.pop(key, None)
    future = asyncio.run_coroutine_threadsafe(bot.delete_messages(chat_id, message_id), MAIN_LOOP)
...
def telegram_bot_file_url(file_id):
    """Get Telegram Bot API file URL. Bot API downloads are limited to 20 MB."""
    url = "https://api.telegram.org/bot{}/getFile?file_id={}".format(
        urllib.parse.quote(BOT_TOKEN, safe=""),
        urllib.parse.quote(str(file_id), safe=""),
    )
    req = urllib.request.Request(url, headers={"User-Agent": "TrueStream/8"}, method="GET")
    with urllib.request.urlopen(req, timeout=12) as r:
        data = json.loads(r.read().decode("utf-8"))
    if not data.get("ok") or not data.get("result", {}).get("file_path"):
        raise RuntimeError("Telegram Bot API getFile failed")
    return "https://api.telegram.org/file/bot{}/{}".format(
        BOT_TOKEN, data["result"]["file_path"]
    )


def botapi_stream_sync(file_url, start, length, out_queue, stop_event):
    """Blocking Bot API download used only for files <=20 MB."""
    end = start + length - 1
    headers = {
        "User-Agent": "TrueStream/8",
    ...
    with urllib.request.urlopen(req, timeout=20) as r:
        ...
        if not partial and start:
            to_skip = start
            while to_skip > 0 and not stop_event.is_set():
                data = r.read(min(CHUNK_SIZE, to_skip))
                if not data:
                    break
                to_skip -= len(data)
        while sent < length and not stop_event.is_set():
            data = r.read(min(CHUNK_SIZE, length - sent))
            if not data:
                break
            while not stop_event.is_set():
                try:
                    out_queue.put(data, timeout=0.25)
                    break
                except queue.Full:
                    continue
...
async def botapi_stream_producer(message, out_queue, stop_event, start, length):
    sent = 0
    try:
        file_id = media_file_id(message)
        if not file_id:
            raise RuntimeError("No Telegram media file_id")
        file_url = await asyncio.to_thread(telegram_bot_file_url, file_id)
        log.info("BOT API DIRECT STREAM start=%s length=%s", start, length)
        sent = await asyncio.to_thread(
            botapi_stream_sync, file_url, start, length, out_queue, stop_event
        )
        log.info("BOT API DIRECT STREAM finished sent=%s/%s", sent, length)
    except asyncio.CancelledError:
        log.info("BOT API DIRECT STREAM CANCELLED sent=%s/%s", sent, length)
        raise
    except Exception:
        log.exception("Telegram Bot API direct stream failed sent=%s/%s", sent, length)
    finally:
        try:
...
class LargeFileCacheEntry:
    def __init__(self, key, part_path, final_path, total_size):
        self.key = key
...
def _cache_key(message, total_size):
    chat_id = getattr(getattr(message, "chat", None), "id", "storage")
    msg_id = getattr(message, "id", getattr(message, "message_id", 0))
    raw = f"{chat_id}:{msg_id}:{total_size}".encode()
    return hashlib.sha256(raw).hexdigest()


def _cache_paths(key):
    return (
        os.path.join(STREAM_CACHE_DIR, key + ".part"),
        os.path.join(STREAM_CACHE_DIR, key + ".bin"),
    )


def _cache_touch(entry):
    with entry.condition:
        entry.last_access = time.time()
        entry.condition.notify_all()


def _cache_existing_size(entry):
    for path in (entry.final_path, entry.part_path):
        try:
            size = os.path.getsize(path)
            if size > entry.total_size:
                with open(path, "r+b") as f:
                    f.truncate(entry.total_size)
...
def get_cache_entry(message, total_size):
    key = _cache_key(message, total_size)
    with CACHE_MAP_LOCK:
        entry = CACHE_ENTRIES.get(key)
        if entry is None:
            part_path, final_path = _cache_paths(key)
            entry = LargeFileCacheEntry(key, part_path, final_path, total_size)
            existing = _cache_existing_size(entry)
            entry.downloaded = existing
            entry.done = os.path.exists(final_path) and existing >= total_size
            CACHE_ENTRIES[key] = entry
        entry.last_access = time.time()
    return entry


def _cache_notify(entry):
    with entry.condition:
        entry.condition.notify_all()
...
def _cache_disk_usage():
    ...
    try:
        for name in os.listdir(STREAM_CACHE_DIR):
            if name.endswith((".part", ".bin")):
                try:
                    total += os.path.getsize(os.path.join(STREAM_CACHE_DIR, name))
                except OSError:
                    pass
    except OSError:
        pass
    ...
    try:
        ...
        for name in os.listdir(STREAM_CACHE_DIR):
            ...
            try:
                ...
                if now - mtime > STREAM_CACHE_TTL:
                    os.remove(path)
            except OSError:
                pass
        usage = _cache_disk_usage()
        if usage <= STREAM_CACHE_MAX_BYTES:
            return
...
async def large_file_cache_worker(message, entry):
    """Keep one long-lived Telegram media generator per large file."""
    stream = None
    try:
        downloaded = _cache_existing_size(entry)
        aligned = downloaded - (downloaded % CHUNK_SIZE)
        if aligned != downloaded:
            try:
                with open(entry.part_path, "r+b") as f:
                    f.truncate(aligned)
            except OSError:
                aligned = 0
        entry.downloaded = aligned

        if entry.done or entry.downloaded >= entry.total_size:
            entry.done = True
            _cache_notify(entry)
            return

        offset = entry.downloaded // CHUNK_SIZE
        log.info("CACHE WORKER START key=%s offset_chunk=%s size=%s", entry.key[:12], offset, entry.total_size)

        while entry.downloaded < entry.total_size:
            while entry.demand_end <= entry.downloaded and not entry.done:
                await asyncio.sleep(0.25)

            target_end = min(entry.total_size, max(entry.demand_end, entry.downloaded + CHUNK_SIZE))
            progress_before = entry.downloaded

            try:
                while entry.downloaded < target_end and entry.downloaded < entry.total_size:
                    try:
                        if stream is None:
                            # Serialize only the first media-DC operation. After
                            # the generator is established, keep using it.
                            async with MT_PROTO_STREAM_LOCK:
                                stream = bot.stream_media(message, offset=offset).__aiter__()
                    ...
                    if not chunk:
                        continue
                    remaining = entry.total_size - entry.downloaded
                    if len(chunk) > remaining:
                        chunk = chunk[:remaining]
                    with open(entry.part_path, "ab") as f:
                        f.write(chunk)
                        f.flush()
                    entry.downloaded += len(chunk)
                    entry.last_access = time.time()
                    try:
                        os.utime(entry.part_path, None)
                    except OSError:
                        pass
                    _cache_notify(entry)
            except asyncio.CancelledError:
                ...
                if "FLOOD_WAIT" in str(exc).upper():
                    log.error("CACHE MTProto FloodWait %ss; cache worker stopped", wait_seconds or "unknown")
                    raise
                # A broken media generator is recreated from the current aligned
                # cache position. It is not recreated for ordinary Range requests.
                stream = None
                if entry.downloaded > progress_before:
                    offset = entry.downloaded // CHUNK_SIZE
                    continue
                raise

            if entry.downloaded >= entry.total_size:
                try:
                    os.replace(entry.part_path, entry.final_path)
                except OSError:
                    pass
                entry.downloaded = entry.total_size
                entry.done = True
                log.info("CACHE DOWNLOAD FINISHED key=%s size=%s", entry.key[:12], entry.total_size)
                _cache_notify(entry)
                return

            if entry.downloaded <= progress_before and stream is None:
                raise RuntimeError(f"Telegram stream made no progress at {entry.downloaded}/{entry.total_size}")
            offset = entry.downloaded // CHUNK_SIZE
        ...
        entry.error = str(exc)
        log.exception("CACHE DOWNLOAD FAILED key=%s downloaded=%s/%s", entry.key[:12], entry.downloaded, entry.total_size)
    finally:
        ...
        entry.task = None
        _cache_notify(entry)
        await asyncio.to_thread(_cache_cleanup)


def ensure_cache_worker(message, entry):
    entry.last_access = time.time()
    if entry.done or (entry.task and not entry.task.done()):
        return
    entry.error = None
    entry.task = asyncio.create_task(large_file_cache_worker(message, entry))


def read_cached_range(entry, start, length, out_queue, stop_event):
    sent = 0
    path = entry.final_path if entry.done else entry.part_path
    with open(path, "rb") as f:
        f.seek(start)
        while sent < length and not stop_event.is_set():
            data = f.read(min(CHUNK_SIZE, length - sent))
            if not data:
                break
            while not stop_event.is_set():
                try:
...
async def stream_producer(message, out_queue, stop_event, start_chunk, first_skip, wanted_bytes):
    """Serve a large file from a single persistent MTProto-backed cache."""
    sent = 0
    absolute_start = start_chunk * CHUNK_SIZE + first_skip
    total_size = int(getattr(getattr(message, "video", None), "file_size", 0) or 0)
    for attr in ("document", "audio", "photo"):
        if total_size:
            break
        total_size = int(getattr(getattr(message, attr, None), "file_size", 0) or 0)
    if not total_size:
        raise RuntimeError("Unknown Telegram media size")

    entry = get_cache_entry(message, total_size)
    with entry.condition:
        entry.demand_end = max(entry.demand_end, min(total_size, absolute_start + wanted_bytes))
        entry.last_access = time.time()
        entry.condition.notify_all()
    ensure_cache_worker(message, entry)
    log.info("STREAM CACHE START key=%s start=%s wanted=%s cached=%s/%s demand=%s", entry.key[:12], absolute_start, wanted_bytes, entry.downloaded, total_size, entry.demand_end)

    try:
        # Wait until the requested bytes have arrived in the cache.
        while sent < wanted_bytes and not stop_event.is_set():
            needed = absolute_start + wanted_bytes
            entry.last_access = time.time()
            available = entry.downloaded
            if entry.error and available < needed:
                raise RuntimeError(entry.error)
            if available < needed and not entry.done:
        ...
        log.info("STREAM CACHE FINISHED sent=%s/%s", sent, wanted_bytes)
    except asyncio.CancelledError:
        # The cache worker deliberately continues. The next Range request can
        # reuse the same file without another Telegram media-DC authorization.
        log.info("STREAM CACHE CLIENT CANCELLED sent=%s/%s", sent, wanted_bytes)
        raise
    except Exception:
        log.exception("STREAM CACHE FAILED sent=%s/%s", sent, wanted_bytes)
        raise
    finally:
        try:
            out_queue.put_nowait(None)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def serve_media(self, item, download=False):
        try:
            ...
            # Telegram Bot API can download files up to 20 MB. Use that path for
            # small videos/audio so playback does not depend on Pyrogram's media
            # DC upload.GetFile connection, which is the source of the observed
            # Request timed out / retry storm on Render. Larger files continue
            # through the MTProto streamer.
            if size <= BOT_API_MAX_DOWNLOAD:
                producer = botapi_stream_producer(msg, q, stop_event, start, length)
                log.info("MEDIA PATH=BOT_API size=%s start=%s length=%s", size, start, length)
            else:
                producer = stream_producer(msg, q, stop_event, start_chunk, first_skip, length)
                log.info("MEDIA PATH=MTProto size=%s start=%s length=%s", size, start, length)

            future = asyncio.run_coroutine_threadsafe(producer, MAIN_LOOP)
    ...
    def do_GET(self):
        ...
        if path == "/health":
            ...
            return
        if path == "/":
            self.send_text(200, "True Stream V8 is running\n")
            return
