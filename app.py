...
def get_storage_message(item):
    ...
    # This is called from HTTP threads, so bridge into the Pyrogram main loop.
    future = asyncio.run_coroutine_threadsafe(bot.get_messages(chat_id, int(item["storage_message_id"])), MAIN_LOOP)
...
def webpage(item, code):
    ...
    if is_video:
        preview = f'<video controls playsinline preload="metadata" src="{html.escape(stream_url, quote=True)}"></video>'
...
async def stream_producer(message, out_queue, stop_event, start_chunk, first_skip, wanted_bytes):
    ...
    try:
        async for chunk in bot.stream_media(message, offset=start_chunk):
            ...
            if chunk:
                await asyncio.to_thread(out_queue.put, chunk)
                sent += len(chunk)
            if sent >= wanted_bytes:
                break
    except Exception:
        log.exception("Telegram media streaming failed")
    finally:
        await asyncio.to_thread(out_queue.put, None)
